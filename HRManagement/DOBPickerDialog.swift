...
private extension Selector {
    static let buttonTapped = #selector(DOBPickerDialog.buttonTapped)
    static let deviceOrientationDidChange = #selector(DOBPickerDialog.deviceOrientationDidChange)
}

open class DOBPickerDialog: UIView {
    public typealias DatePickerCallback = ( String? ) -> Void
    ...
    open func show(_ title: String, doneButtonTitle: String = "Done", cancelButtonTitle: String = "Cancel", defaultDate: String?, callback: @escaping DatePickerCallback) {
        ...
        if self.defaultDate != nil {
            ...
            if let unwrappedDate = df.date(from: self.defaultDate!) {
    ...
    @objc func buttonTapped(sender: UIButton!) {
        if sender.tag == kDatePickerDialogDoneButtonTag {
            let df = DateFormatter()
            df.dateFormat = "yyyy-MM-dd"
            self.callback?(df.string(from: self.datePicker.date))
        } else {
            self.callback?(nil)
        }
